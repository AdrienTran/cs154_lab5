...
forward_A = pyrtl.WireVector(bitwidth=2, name='forward_A')
forward_B = pyrtl.WireVector(bitwidth=2, name='forward_B')

with pyrtl.conditional_assignment:
    # EX hazard
    with (reg_write_xm & (rd_xm != 0) & (rd_xm == rs_d)):
        forward_A |= 2
    with (reg_write_xm & (rd_xm != 0) & (rd_xm == rt_d)):
        forward_B |= 2
        
    # MEM hazard
    with (reg_write_mw & (rd_mw != 0) & ~(reg_write_xm & (rd_xm != 0) & (rd_xm == rs_d)) & (rd_mw == rs_d)):
        forward_A |= 1
    with (reg_write_mw & (rd_mw != 0) & ~(reg_write_xm & (rd_xm != 0) & (rd_xm == rt_d)) & (rd_mw == rt_d)):
        forward_B |= 1
...
with pyrtl.conditional_assignment:
    ...
    with pyrtl.otherwise:
        ALU_first_x |= rf_data_1_dx

    with forward_B == 1:
        ALU_second_x |= rf_write_data_w
    with forward_B == 2:
        ALU_second_x |= ALU_result_xm
    with pyrtl.otherwise:
        with ALU_src_dx==0:
